...
send_pid:
		...
		case ANTO_STATUS:     //send angle
	      ...
	      Anto[5] = (int16_t)(0);			//高度
			...
			break;
		case ANTO_POWER:
				Anto[2] = (int16_t)(4.2*100);  //电压
		    Anto[3] = (int16_t)(0*100);
		    len = 2;
				break;
		case ANTO_SENSER2:
			
//				long _temp = (long)FlightData.High.bara_height;  //激光高度
//				*(uint16_t*)&Anto[2] = *((uint16_t*)&_temp+1);
//				*(uint16_t*)&Anto[3] = *((uint16_t*)&_temp);
//				Anto[4] = (uint32_t)FlightData.High.ultra_height; //超声波高度
//				len = 6;
