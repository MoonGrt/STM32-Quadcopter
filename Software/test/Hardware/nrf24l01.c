...
u8 NRF24L01_Write_Reg(u8 regaddr, u8 data)
{
    ...
    status = SPI_SwapByte(regaddr); // 发送寄存器号
...
u8 NRF24L01_Read_Buf(u8 regaddr, u8 *pBuf, u8 datalen)
{
    u8 status, u8_ctr;
    Clr_NRF24L01_CSN;               // 使能SPI传输
    status = SPI_SwapByte(regaddr); // 发送寄存器值(位置),并读取状态值
    for (u8_ctr = 0; u8_ctr < datalen; u8_ctr++)
        pBuf[u8_ctr] = SPI_SwapByte(0XFF); // 读出数据
